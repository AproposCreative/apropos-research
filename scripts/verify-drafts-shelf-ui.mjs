...
const source = await readFile('components/DraftsShelf.tsx', 'utf8') + await readFile('components/MobileAppLauncher.tsx', 'utf8');
...
const bundle = await build({stdin:{resolveDir:process.cwd(),loader:'tsx',contents:`
 ...
 import Launcher from './components/MobileAppLauncher';
 ...
 function Flow(){const [open,setOpen]=useState(false);return open?<Shelf
...
`},bundle:true,write:false,jsx:'automatic',format:'iife',define:{'process.env.NODE_ENV':'"development"'},plugins:[{name:'fixtures',setup(b){
 ...
 return React.createElement(Context.Provider,{value:{user:uid?{uid}:null,capabilities:{owner:true}}},children);}
 `:`
...
const logo = await readFile('public/images/Apropos Research White.png');
createServer((req,res)=>{
 if(req.url==='/images/Apropos%20Research%20White.png'){res.setHeader('Content-Type','image/png');res.end(logo);return;}
