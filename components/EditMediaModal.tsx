...
interface EditMediaModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (source: MediaSource) => void;
  source: MediaSource;
}
...
export default function EditMediaModal({ isOpen, onClose, onSuccess, source }: EditMediaModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Fejl ved opdatering af mediekilde');
      }

      // Update the source object with new data
      const updatedSource = {
        ...source,
        name: formData.name,
        baseUrl: formData.baseUrl,
      ...
      if (typeof window !== 'undefined') {
        ...
        notification.innerHTML = `
          <div style="
            ...
            max-width: 300px;
          ">
            ✅ ${updatedSource.name} opdateret!<br>
      ...
      console.log('Starting background ingest for updated media source...');
      fetch('/api/refresh', {
        method: 'POST',
      })
      .then(ingestResponse => {
          ...
          if (typeof window !== 'undefined') {
            ...
            notification.innerHTML = `
              <div style="
                ...
                max-width: 300px;
              ">
                🎉 Artikler opdateret!<br>
                <small style="opacity: 0.9;">${updatedSource.name} er klar til brug</small>
              </div>
            `;
            document.body.appendChild(notification);
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              className="flex-1 px-4 py-3 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-slate-400"
              disabled={isLoading || (validationResults && !validationResults.hasArticles)}
            >
              {isLoading ? 'Opdaterer...' : 'Opdater mediekilde'}
            </button>
          </div>
        </form>
      </div>
