  ...
  const handleSendToAI = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Show success modal with slight delay to ensure proper rendering
  ...
  if (editorialItems.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="bg-slate-100/50 dark:bg-pure-black/50 backdrop-blur-sm rounded-2xl p-8 border border-slate-200/50 dark:border-slate-700/50">
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2 bg-slate-100/70 dark:bg-black/70 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200/70 dark:hover:bg-slate-700/70 transition-colors duration-300 border border-slate-300/50 dark:border-slate-600/50"
            >
              Kopiér valgte
            </button>
            <button
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-2xl p-4 border border-slate-200/50 dark:border-slate-700/50 shadow-lg">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white/70 dark:bg-black/70 border border-slate-300/50 dark:border-slate-600/50 rounded-xl text-slate-700 dark:text-slate-300 placeholder-slate-500 dark:placeholder-slate-400 focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400 focus:border-transparent transition-all duration-300"
            />
          </div>
        </div>
      )}
      ...
      <div className="grid gap-4">
        {filteredItems.map((item, index) => (
          <div 
            key={item.id}
            className="bg-white/80 dark:bg-pure-black/80 backdrop-blur-2xl rounded-2xl p-6 border border-slate-200/50 dark:border-black-800/50 shadow-lg hover:shadow-xl transition-all duration-300"
          >
            {/* Ticket Header */}
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-gradient-to-r from-black-600 to-black-700 text-white rounded-full flex items-center justify-center text-sm font-semibold">
                  {index + 1}
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-slate-800 dark:text-black-100 line-clamp-2">
                    {item.title}
                  </h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="px-2 py-1 bg-slate-100/70 dark:bg-pure-black/70 text-slate-600 dark:text-black-400 text-xs rounded-full border border-slate-300/50 dark:border-black-700/50">
                      {item.source?.toUpperCase() || 'MEDIE'}
                    </span>
                    <span className="text-xs text-slate-500 dark:text-black-500">
              ...
              <div className="text-right">
                <div className="text-sm text-slate-500 dark:text-black-500">
      ...
      {showSuccessModal && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '24px', color: 'green', marginBottom: '10px' }}>✓</div>
            <h3 style={{ marginBottom: '10px', fontSize: '18px' }}>Artikler sendt til AI!</h3>
            <p style={{ marginBottom: '20px', whiteSpace: 'pre-line' }}>{successMessage}</p>
