...
import { workspaceSnapshotSchema, type WorkspaceSnapshot } from '@/lib/writer-workspace';

type Version = { id: string; kind: 'history' | 'conflicts'; title: string; updatedAt: string; revision: number };
/** Read-only recovery view. Opening/exporting a version does not overwrite work. */
export default function WorkspaceVersions({ onClose }: { onClose: () => void }) {
  const { user } = useAuth();
  ...
  return <dialog ref={dialog} onCancel={onClose} aria-labelledby="workspace-versions-title" className="fixed inset-4 m-0 h-[calc(100%-2rem)] max-h-none w-[calc(100%-2rem)] max-w-none flex-col overflow-hidden rounded-xl border border-white/20 bg-[#101010] p-5 text-white shadow-2xl backdrop:bg-black/70 open:flex md:inset-16 md:h-[calc(100%-8rem)] md:w-[calc(100%-8rem)]">
    <header className="flex items-center justify-between gap-3"><h2 id="workspace-versions-title" className="text-lg">Mine gemte versioner</h2><button autoFocus className="min-h-11 px-3" onClick={onClose}>Luk</button></header>
    ...
    <div className="min-h-0 overflow-y-auto">
      {choice && <button className="min-h-11 underline" onClick={() => setChoice(null)}>← Til listen</button>}
