...
export default function LivDeskClient({ onClose, onOpenWriter }: { onClose: () => void; onOpenWriter: (story: DeskStory) => void }) {
  ...
  const refresh = useCallback(async () => {
    try { const data = await request(); setStories(data.stories); setAudience(data.audience || null); setLoaded(true); }
    catch (e) { setError(e instanceof Error ? e.message : 'Der opstod en fejl.'); }
  }, [request]);
  useEffect(() => { void refresh(); }, [refresh]);
  async function act(action: string, id?: string) {
    setBusy(true); setError('');
    try { await request({ action, id }); await refresh(); }
    catch (e) { setError(e instanceof Error ? e.message : 'Der opstod en fejl.'); await refresh(); }
