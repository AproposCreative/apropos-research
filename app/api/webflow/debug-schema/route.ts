...
function resolveConfig() {
  ...
  const articlesCollectionId = (file.articlesCollectionId !== undefined ? file.articlesCollectionId : process.env.WEBFLOW_ARTICLES_COLLECTION_ID) || '67dbf17ba540975b5b21c2a6';
  ...
  try {
    ...
    if (!token) {
      return NextResponse.json({
        ...
        hint: 'Check your .env.local file for WEBFLOW_API_TOKEN or WEBFLOW_TOKEN'
      }, { status: 400 });
    }
    ...
    if (!siteId) {
      return NextResponse.json({
        ...
        hint: 'Check your .env.local file for WEBFLOW_SITE_ID'
      }, { status: 400 });
    }

    console.log('🔍 Fetching Webflow schema for collection:', articlesCollectionId);
    ...
    if (!schemaRes.ok) {
      ...
      return NextResponse.json({
        error: 'Failed to fetch Webflow schema',
    ...
    console.error('Error fetching Webflow schema:', error);
    return NextResponse.json({
      error: 'Failed to fetch Webflow schema',
