...
export async function optimizeAndUploadImage(
  ...
  const imageBuffer = await downloadImage(options.imageUrl, Math.max(5000, Math.round(options.fetchTimeoutMs ?? 30000)));
  return optimizeAndUploadImageBytes(imageBuffer, options);
}

/** Server-owned generated bytes only; remote URLs still pass downloadImage validation. */
export async function optimizeAndUploadImageBytes(
  imageBuffer: Buffer,
  options: OptimizeAndUploadImageOptions
): Promise<OptimizeAndUploadImageResult> {
  if (!Buffer.isBuffer(imageBuffer) || !imageBuffer.length || imageBuffer.length > 24 * 1024 * 1024) throw new Error('invalid_generated_image_bytes');
