...
export async function enqueueSeoEngineJob(args: {
  ...
  kickSeoEngineJob({ jobId: written.jobId, itemId: args.itemId });
  return written;
}

/** Kick the existing durable job without rebuilding its id, locale or attempt count. */
export function kickSeoEngineJob(args: { jobId: string; itemId: string }): void {
  const base = resolveInternalBaseUrl();
  const url = `${base}/api/internal/seo-engine-article`;
  void fetch(url, {
    method: 'POST',
    headers: internalApiHeaders(),
    body: JSON.stringify({ jobId: args.jobId, itemId: args.itemId }),
