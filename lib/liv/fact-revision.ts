  ...
  if (!patchResult) {
    ...
    const response = await client.chat.completions.create({ model: livModels().utility, reasoning_effort: 'low',
      max_completion_tokens: 10000, response_format: { type: 'json_object' }, messages: [
        ...
        { role: 'system', content: 'Når en perifer faktapåstand mangler belæg, så fjern den påstand helt og bevar den selvstændige analyse. Erstat den ikke med research-forbehold som "kilderne omtaler ikke", "vi har ikke dokumentation" eller en ny negativ faktapåstand. Hvis påstanden er historiens centrale præmis, må du ikke skjule problemet med en hypotese eller et forbehold.' },
