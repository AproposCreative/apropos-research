...
The September 21 pilot exposed completed semantic-review responses with invalid evidence being conflated with unknown provider outcomes. Keep both blocked, but distinguish them: only a completed, archived invalid response can enter the existing single durable originality rewrite automatically. A timeout still cannot. Scheduled API recovery can reuse the exact saved original writing run after a recorded similarity failure; parent/child receipts still forbid another originality rewrite and every rewritten version must pass all gates. No new research or third candidate is needed to resume that saved work.

The saved Ed Sheeran alternative passed the writing stage after that correction. Its three paid illustrations were then rejected solely for one inaccurate alt description, although the visual verdict found them relevant and without defects. Initial media preparation now uses the same bounded label-repair approach as fact revisions: one persisted description-only correction, followed by one independent review. It reuses all pixels and preserves the original rejected verdict, labels, credits and generation receipts. Unfixable images, invalid responses, failed revised review and uncertain calls stay blocked without another purchase. It does not generate replacement images or modify article prose.
