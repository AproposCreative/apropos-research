...
- Release `1defda5` reached READY on `dpl_C7LfRnbgvxjxTZHyuG12GuhnbS1U`;
  ...
  denials and invalid input checks, but the ready Lucian Freud draft returned
  `409 liv_shortening_draft_changed`. Thus release is live, but that article's
  shortening eligibility is not yet verified. GET-only diagnosis found standard
  Webflow figure wrappers and removed inter-block serialization whitespace.
  A local narrow normalization fix preserves unknown markup and inline spacing;
  29 focused tests pass. That fix still requires regression/build/redeployment.
  Operations at 2026-09-14T03:25:21.278Z: autopublish enabled, Lucian Freud ready,
  no blocked items, not overdue, no publication yet today. No article changed or
  generated by verification. Initial deployment-scoped error/fatal count was empty.
