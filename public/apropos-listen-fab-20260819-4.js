(function () {
  ...
  function ensureCss() {
    ...
    style.id = STYLE_ID;
    style.textContent = [
      /* Animatable angle for rotating conic border */
      ...
      /* Rotating glow behind the pill */
      "#" + FAB_ID + "::before{",
      "content:'';position:absolute;inset:-1px;border-radius:inherit;z-index:-2;",
      "background:conic-gradient(from var(--apropos-listen-a),",
      "#6366f1,#ec4899,#38bdf8,#a78bfa,#6366f1);",
      "opacity:.9;",
      "animation:apropos-listen-spin 3s linear infinite;",
      "}",

      /* Glass fill masks the center so only a thin border glow shows */
      "#" + FAB_ID + "::after{",
      ...
      "#" + FAB_ID + ":hover::after{background:rgba(18,18,18,0.88);border-color:rgba(255,255,255,0.2);}",
