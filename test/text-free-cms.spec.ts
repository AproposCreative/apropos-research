...
it('leaves clean HTML exactly unchanged', async () => {
  f.clean.mockResolvedValue({ bytes: Buffer.from('image'), receipt: { edited: false } });
...
it('preserves a CDN URL for identical already-cleaned bytes instead of creating a webhook loop', async () => {
  f.clean.mockResolvedValue({ bytes: Buffer.from('image'), receipt: { edited: true, image: { url: 'https://storage.test/original-clean.webp' } } });
  const fields = { thumb: { url: 'https://cdn.test/copy.webp', alt: 'Reacher' } };
  await enforceTextFreeArticleImages(fields);
  expect(fields.thumb.url).toBe('https://cdn.test/copy.webp');
});
