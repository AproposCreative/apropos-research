...
const mock = vi.hoisted(() => ({ auth: vi.fn(), state: vi.fn(), payload: vi.fn(), decide: vi.fn(), cost: vi.fn(), preparation: vi.fn(),hold:vi.fn() }));
vi.mock('@/lib/ai/provider-hold',()=>({readProviderHold:mock.hold}));
...
beforeEach(() => {
  ...
  mock.hold.mockResolvedValue({blocked:false});
...
it('shows the actual shared provider stop without hiding ready articles or changing their delivery flags',async()=>{
 mock.hold.mockResolvedValue({blocked:true});
 const data=await(await GET(request())).json();
 expect(data.preparation).toMatchObject({reasonCode:'provider_quota_exhausted',nextAction:'blocked',nextAttemptAt:null});
 expect(data.stories).toHaveLength(7);expect(data.queueEnabled).toBe(true);expect(mock.decide).not.toHaveBeenCalled();
});
it('does not mask a CMS reconciliation or hide the queue when provider-status reading fails',async()=>{
 mock.hold.mockResolvedValue({blocked:true});mock.preparation.mockResolvedValue({status:'reconciliation_required',stage:'cms',reasonCode:'cms_reconciliation_required'});
 expect((await(await GET(request())).json()).preparation.reasonCode).toBe('cms_reconciliation_required');
 mock.hold.mockRejectedValue(Error('store failure'));
 expect((await(await GET(request())).json()).stories).toHaveLength(7);
});
