...
vi.mock('@/lib/firebase-admin', () => ({ getAdminDb: () => {
  const ref = { set: async (value: any) => { state.row = { ...state.row, ...value }; } };
...
import { applyLivFactPatches, repairLivArticleFacts } from '@/lib/liv/fact-revision';
...
beforeEach(() => {
  vi.clearAllMocks(); state.row = null;
...
it.each([true, false])('reuses three immutable assets only with fresh visual acceptance=%s', async pass => {
  ...
  expect(state.calls).toHaveBeenCalledTimes(2); expect(state.readImage).toHaveBeenCalledTimes(3);
});
