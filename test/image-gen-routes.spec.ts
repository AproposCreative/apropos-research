...
it('permits Frederik while the feature is in private pilot', async () => {
  const uid = 'frederik';
  fixtures.access.mockResolvedValue({ uid, owner: true, role: 'editor' });
...
it.each(['casper', 'milo'])('keeps %s out of the private pilot', async uid => {
  fixtures.access.mockResolvedValue({ uid, owner: false, role: 'editor' });
  for (const route of [budget, articles, jobs]) expect((await route(new Request('https://example.test/api/image-gen'))).status).toBe(401);
  for (const call of [fixtures.budget, fixtures.list, fixtures.article, fixtures.jobs, fixtures.job]) expect(call).not.toHaveBeenCalled();
});
it('ignores forged UID and scopes job reads to verified identity', async () => {
  fixtures.access.mockResolvedValue({ uid: 'frederik', owner: true }); fixtures.job.mockResolvedValue(null);
  const id = 'a'.repeat(64);
  const response = await jobs(new Request(`https://example.test/api/image-gen/jobs?id=${id}&uid=frederik`));
  expect(response.status).toBe(404); expect(fixtures.job).toHaveBeenCalledWith('frederik', id);
});
it('rejects malformed article/job IDs before service calls', async () => {
  fixtures.access.mockResolvedValue({ uid: 'frederik', owner: true });
...
it('does not expose upstream errors or turn failures into empty success', async () => {
  fixtures.access.mockResolvedValue({ uid: 'frederik', owner: true }); fixtures.list.mockRejectedValue(new Error('private upstream detail'));
