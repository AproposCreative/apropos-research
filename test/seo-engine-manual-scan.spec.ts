...
vi.mock('@/lib/seo-engine/opportunity-engine/apply', () => ({ maybeAutoApplyOpportunities: vi.fn() }));
...
it('defaults to collect without CMS writes', async () => {
  ...
  expect(maybeAutoApplyOpportunities).not.toHaveBeenCalled();
});
...
it('requires explicit apply opt-in', async () => {
  ...
  expect(maybeAutoApplyOpportunities).toHaveBeenCalledOnce();
});
